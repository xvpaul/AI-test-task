...
while [ "$(date +%s)" -lt "${deadline}" ]; do
  ...
  #
  # Логи кладём в переменную и ищем средствами самого bash, без `| grep -q`.
  # Пайп здесь давал ложное «не готов» при живом шлюзе: grep -q выходит по
  # первому совпадению, пишущий в закрытую трубу docker logs получает SIGPIPE,
  # и при включённом pipefail успешное совпадение превращалось в ненулевой код.
  # Условие молча становилось ложным — деплой падал по таймауту на работающем
  # сервисе. Без пайпа этот класс ошибок исчезает целиком.
  logs="$(docker logs "${cid}" 2>&1 || true)"
  if [ "${running}" = "true" ] && [[ "${logs}" == *"[gateway] ready"* ]]; then
    status="ok"; break
  fi
...
case "${status}" in
  ...
  *)
    ...
    if [[ "${logs:-}" == *"[gateway] ready"* ]]; then
      echo "       строка готовности В ЛОГАХ ЕСТЬ — значит сломана проверка, а не шлюз" >&2
    else
      echo "       строки готовности в логах действительно нет" >&2
    fi
    exit 1
    ;;
esac
